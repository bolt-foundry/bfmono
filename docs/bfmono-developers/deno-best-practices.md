...
Define dependencies centrally in `deno.jsonc`:
...
2. **Lock dependencies** - Commit `deno.lock` to version control
3. **Cache dependencies** - Use `deno cache` before deployment
4. **Version explicitly** - Specify exact versions for stability

```typescript
// Preferred - JSR package
import { assertEquals } from "@std/assert@1.0.0";

// When needed - npm compatibility
