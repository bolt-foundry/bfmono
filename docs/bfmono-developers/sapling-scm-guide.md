...
development. It offers a streamlined workflow optimized for performance at scale
and works seamlessly with existing Git repositories.

## Core Concepts

### 1. Mutable Commits

Sapling treats commits as mutable, making it easy to amend and rebase without
complex operations. This enables a more fluid development workflow.

### 2. Bookmarks Instead of Branches

Sapling uses lightweight bookmarks that point to commits, providing a simpler
model than traditional branching.

### 3. Stacked Development

Sapling excels at managing stacks of related commits, making it natural to work
on multiple dependent changes simultaneously.

### 4. Performance at Scale

Optimized for large monorepos with features like lazy fetching and efficient
history operations
...
```bash
# Clone a repository
sl clone <repository-url>

# Initialize in existing directory
sl init --git

# Work with Sapling commands
sl status
sl commit
sl push
```
...
- `sl status` - Show working directory status
- `sl add` - Stage files for commit
- `sl commit` - Create a new commit
- `sl log` - View commit history
- `sl bookmarks` - Manage bookmarks
- `sl goto` - Navigate to a commit or bookmark
- `sl pull` - Fetch and integrate changes
- `sl push` - Upload commits to remote
- `sl rebase` - Reapply commits on new base
- `sl shelve` - Temporarily store changes
- `sl graft` - Copy commits to current location
