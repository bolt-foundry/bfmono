...
Deno.test("GraphQL Connection Integration - connection preserves node properties and metadata", async () => {
  await withIsolatedDb(async () => {
    ...
    // Using simplified telemetry data structure for testing
    const telemetryData = edgeNode.props.telemetryData as {
      id: string;
      model: string;
      usage: {
    ...
    assertEquals(telemetryData.model, "gpt-4-turbo");
    assertEquals(telemetryData.usage.total_tokens, 300);
    assertEquals(
      telemetryData.choices[0].message.content,
