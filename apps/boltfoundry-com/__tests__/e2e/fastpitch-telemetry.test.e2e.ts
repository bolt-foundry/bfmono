...
import { generate as fastpitchGenerate } from "@bfmono/infra/bft/tasks/fastpitch.bft.ts";
...
async function mockOpenRouterResponse() {
...
Deno.test("Phase 1 & 2: Fastpitch telemetry to dashboard flow", async (t) => {
  await withIsolatedDb(async () => {
    ...
    try {
      ...
      await t.step("1. Navigate to homepage and login", async () => {
        ...
        const orgData = await context.page.evaluate(() => {
      ...
      await t.step("3. Run Fastpitch generate", async () => {
        ...
        const originalBfApiKey = Deno.env.get("BF_API_KEY");
        const originalOpenRouterKey = Deno.env.get("OPENROUTER_API_KEY");
        const originalBfBaseUrl = Deno.env.get("BF_BASE_URL");
        ...
        try {
          ...
          if (originalBfBaseUrl) Deno.env.set("BF_BASE_URL", originalBfBaseUrl);
      ...
      await t.step("4. Verify deck appears in dashboard", async () => {
        ...
        const deckExists = await context.page.evaluate(() => {
          const deckElements = Array.from(
            document.querySelectorAll('[class*="deck"], [class*="list-item"]'),
          );
        ...
        if (deckExists) {
          ...
          const deckClicked = await context.page.evaluate(() => {
            const deckElements = Array.from(
          ...
          if (deckClicked) {
            ...
            const hasRealContent = await context.page.evaluate(() => {
          ...
          const deckExistsAfterRefresh = await context.page.evaluate(() => {
            const deckElements = Array.from(
              document.querySelectorAll(
                '[class*="deck"], [class*="list-item"]',
              ),
