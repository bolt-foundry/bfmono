...
- `createdDeckId: string` - Deck ID from Phase 2
...
2. **Find Test Deck**
   ```typescript
   // Current implementation doesn't have data-deck-id attributes
   // Need to find deck by text content or add attributes

   // Option 1: Find by deck name (current approach)
   const deckSelector = `.deck-card:has-text("${createdDeckName}")`;

   // Option 2: Find by deck ID if attributes are added
...
2. **Verify Sample Metadata**
   ...
   assert(deckName?.includes(createdDeckName), "Correct deck loaded");
   ```
...
1. **Add Data Attributes:**
   - `data-deck-id` to deck cards in DeckList
